...
def deploy_runtime(q):
    ...
    env_vars = {
        "GATEWAY_URL": q["gateway_url"],
        "BEDROCK_MODEL_ID": q["bedrock_model_id"],
        # Routes OTEL traces/prompts to the unified, resource-specific log
        # group instead of a generic one — see aws_agentcore.tf's
        # aws_cloudwatch_log_delivery_* wiring, which is what actually
        # populates CloudWatch GenAI Observability's Agents dashboard.
