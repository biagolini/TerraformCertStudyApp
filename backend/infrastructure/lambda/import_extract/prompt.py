...
- IGNORE any past-test-taker status labels near the question (e.g. "Incorreto", "Correto", "Ignorado", "Ignored", or a lone "Correct"/"Incorrect" placeholder line sitting next to an option with no other content) — these describe what some OTHER person answered before on this exam attempt, NOT the true correct answer. This noise is NOT limited to standalone lines — it very often sits GLUED directly in front of the real scenario text with no line break at all, as one run-on chunk, e.g. "Pergunta 3 Ignorado Explique melhor A global financial services company manages..." or "Question 7 Correct Explain this further As a DevOps Engineer...". In that exact pattern, the true stem begins ONLY at "A global financial..."/"As a DevOps Engineer..." — the leading "Pergunta N"/"Question N" heading number, the status word ("Ignorado"/"Correto"/"Incorreto"/"Correct"/"Incorrect"), and the "Explique melhor"/"Explain this further" link text are ALL UI chrome from the source export and must never appear anywhere in the extracted stem, not even as its first few words. Read past all of it to find where the actual scenario sentence starts.
- IGNORE a leading "Category:" line some sources place between the question heading and the real stem, such as "Category: DOP – SDLC Automation" or "Category: Governance and Responsible AI". Never include it in the stem — the scenario begins only AFTER it. You may use it as a hint for the "domain" field, but the stem itself must start at the real scenario sentence.
...
- Detect the natural language the question itself (stem/alternatives) is actually written in, and report it as an ISO 639-1 code (e.g. "en", "pt", "es", "fr"). Most certification exam questions are in English, but some source material is in another language — never assume English, read the actual text.
                ...
                "json": {
                    ...
                    "properties": {
                        ...
                        "language": {
                            "type": "string",
                            "description": (
                                "ISO 639-1 code (e.g. 'en', 'pt', 'es', 'fr') of the natural "
                                "language the question's stem/alternatives are actually written "
                                "in. Most certification exam questions are in English, but some "
                                "source material is in another language — read the actual text, "
                                "never assume 'en' by default."
                            ),
                        },
                        "stem": {
                            "type": "string",
                    ...
                    "required": ["title", "domain", "language", "stem", "alternatives"],
                }
            },
        }
    }
