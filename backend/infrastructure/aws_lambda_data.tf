...
resource "aws_iam_role_policy" "lambda_data_costs" {
  name = "${var.project_prefix}-lambda-data-costs"
  role = aws_iam_role.lambda_data.id

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      # Query — GET /data/usage's per-user USAGE# scan. UpdateItem — the
      # atomic ADD spentMicros in _add_budget_spend. Neither is part of the
      # grant above — this table needed them and the others don't.
      Effect   = "Allow"
