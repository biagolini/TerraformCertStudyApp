...
# Tracing is ADOT (aws-opentelemetry-distro), not the X-Ray SDK — the
# container starts under `opentelemetry-instrument` (see Dockerfile), which
# auto-instruments boto3/httpx itself. Strands Agents also has built-in OTEL
# telemetry (scope `strands.telemetry.tracer`) that ADOT picks up
# automatically on AgentCore Runtime, giving per-invocation token/cost data
# in CloudWatch GenAI Observability's Agents dashboard — something the old
# X-Ray-SDK segments never surfaced there.
...
def _normalize_usage(raw_usage) -> dict:
    """Strands' `EventLoopMetrics.accumulated_usage`/`AgentResult.metrics.
    accumulated_usage` — normalized to the same Bedrock-shaped
    {inputTokens, outputTokens, totalTokens} dict `/converse` already emits,
    since this app's usage-logging helpers (review/import_explain Lambdas)
    only understand that shape. Defensive about the exact key casing since
    it isn't pinned down by public API docs."""
    if not raw_usage:
        return {}
    d = dict(raw_usage)
    input_tokens = d.get("inputTokens", d.get("input_tokens", 0)) or 0
    output_tokens = d.get("outputTokens", d.get("output_tokens", 0)) or 0
    total_tokens = d.get("totalTokens", d.get("total_tokens", input_tokens + output_tokens)) or 0
...
def _extract_related_services(stem: str, alternatives: list) -> dict:
    ...
    part of the review-writing skill. Returns usage/modelId alongside the
    list (not just the list) so the caller can log this call's own token
    cost separately — it runs on a different, heavier model
    (RELATED_SERVICES_MODEL_ID) than the main review call."""
...
async def invoke(payload: dict):
    ...
    try:
        ...
        try:
            ...
            # Strands' own accumulated token accounting (the same numbers
            # that feed the `gen_ai.usage.*` OTEL span attributes) — surfaced
            # here so the "review" Lambda can log it for the app's own
