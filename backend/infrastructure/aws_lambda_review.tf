...
resource "aws_lambda_function" "review" {
  ...
  environment {
    variables = {
      ...
      # Costs page — logs one usage row per review call, plus the monthly
      # AI budget check (see app.py's _write_usage_event/_check_budget).
      # Read side is lambda/data's GET /data/usage and /data/budget.
...
# --- Costs page — write-only access to log usage rows (see app.py) ---

resource "aws_iam_role_policy" "lambda_review_usage_write" {
  name = "${var.project_prefix}-lambda-review-usage-write"
  role = aws_iam_role.lambda_review.id
  ...
  policy = jsonencode({
    ...
    Statement = [{
      Effect = "Allow"
      # PutItem for USAGE# rows; GetItem/UpdateItem for the monthly AI
      # budget check (BUDGET#CONFIG, BUDGET#{YYYY-MM}) — see review/app.py's
