      ...
      @if (current(); as question) {
        <header class="viewer-header">
          ...
          <div class="header-actions">
            ...
            <button
              type="button"
              class="icon-btn"
              (click)="onToggleTranslate(question)"
              [class.active]="showTranslated()"
              [attr.aria-label]="showTranslated() ? i18n.t('reviewViewer.showOriginal') : i18n.t('reviewViewer.translate')"
              ...
              [disabled]="editing() || refining() || translating()"
            >
              @if (translating()) {
                <mat-spinner diameter="18"></mat-spinner>
              } @else {
                <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
                  <path fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" d="M5 8h9M9.5 5v3M13 8a12 12 0 01-6.5 8M8 12a10.7 10.7 0 006 5M13 20l4-9 4 9M14.7 17h4.6"/>
                </svg>
              }
            </button>
            <button
        ...
        <div class="viewer-body">
          @if (editing()) {
            ...
            @if (translateError()) {
              <p class="edit-error" role="alert">{{ translateError() }}</p>
            }

            <div class="stem"><app-markdown-renderer [source]="displayStem(question)" /></div>
            ...
            <div class="alternatives">
              @for (opt of question.alternatives; track opt.letter; let i = $index) {
                <div class="option-card" [class.correct]="revealAnswers() && opt.isCorrect">
                  <span class="option-letter">{{ opt.letter }}</span>
                  <div class="option-body">
                    <div class="option-text"><app-markdown-renderer [source]="displayAltText(opt, i)" /></div>
            ...
            @if (question.generalComment && revealAnswers()) {
              <div class="general-comment">
                ...
                <app-markdown-renderer [source]="displayGeneralComment(question) ?? ''" />
              </div>
            }
...
export class ReviewViewerComponent {
  ...
  /** Target language shown in the button's tooltip and sent to the model —
   * see resolveTranslationTargetLabel for the automatic-vs-override and
   * don't-translate-into-the-source-language logic. */
  protected readonly translateTargetLabel = computed(() =>
    resolveTranslationTargetLabel(this.question()?.language, this.settings.interfaceLanguage(), this.settings.translationTargetLanguage()),
  );

  /** Translates the current question's content on first click; subsequent
   * clicks just flip between the cached translation and the original — no
   * re-fetch, matching "in place" per the feature request (a quick flip back
   * and forth while studying). */
  ...
  displayAltText(opt: QuestionAlternative, index: number): string {
    return this.showTranslated() ? this.translatedContent()?.alternatives[index]?.text ?? opt.text : opt.text;
  }

  displayAltComment(opt: QuestionAlternative, index: number): string {
